...
### 2.11 验证：门控在提交上，且是 opt-in

**动机来自一次实测。** 一份由真实模型生成、能干净 `git apply`、每处锚点都定位准确的补丁，**仍然过不了 `tsc`（2/7 个文件）和 `vitest`（43/311 个测试失败）**——缺 import、引用了没定义的函数。

也就是说：整套 diff 机制（recount、唯一性预检、重试）验证的是**可行性**，而"改动是对的"是另一件事。没有验证时 `perf run` 会**报告成功**并交出一个破坏构建的补丁，而用户唯一的防线是在预览里逐行看出"这里少了个 import"——那恰恰是 D1 说过"不该让人审"的东西。

**门控点在提交之前。** 每个 step 写出 → 跑验证 → 通过才 `git add`/`commit`。这样分支上只会累积可用的步骤，坏的那步根本不进历史。代价是 N 倍验证时间（调用方可以把命令写成快的，如只跑 `tsc --noEmit`）。

**验证失败时停止，而不是继续。** 后续 step 的 diff 是针对「含这一步」的预测态生成的，而这步没落地——继续只会得到一堆误导性的"锚点对不上"，掩盖真正的原因。失败的那步**留在工作区不撤销**：验证的价值就是让用户知道哪里坏了，代码留在原处才便于排查；分支本身仍然干净。

**命令由配置提供、由 `--verify` 武装。** 这个拆分是安全要求：`.perftoolrc.json` 会随仓库一起被克隆/分发，所以"配置里写了命令"与"用户同意执行它"必须是两件事——否则一个恶意配置就变成了任意代码执行。`--yes` 不隐含 `--verify`。这与 `--yes` 之于改用户代码是同一条原则。

**这打破了"绝不执行目标项目里的任何命令"那条承诺，所以它被写成了显式例外**，并同步改进了 `README` 的安全一节。

**门槛的强度就是配置里那条命令的强度——这是实测出来的。** 一次真实的 `perf run --verify`（配置 `verify: "npm run typecheck"`）里：

- 第 3 步产出的改动有一个未配对的 `}`，`tsc` 报 TS1005 —— **门控挡住了它，它没进历史**；
- 但另两步通过了 `tsc` 却被放行，事后跑测试发现其中一步有真实语义退化：它缓存了整树枚举结果，于是**探索期间新建的文件不再可见**。讽刺的是模型在计划的 rationale 里自己预见并接受了这个不一致（原文写着"read_file 会看到新内容——这个不一致要显式承认"）。

两个教训：① 门控的机制是对的，但"配什么命令"决定了它能挡什么——`tsc` 挡不住行为变化；② AI 产出的性能改动会引入**它自己已经预见却仍然接受**的语义折衷，而这类折衷只有测试能挡。

### 2.12 多轮运行的合并（`--runs N`）—— 设计已定，尚未实现

**动机是一次测量。** 同一个仓库上跑了四次 `perf plan`（前两次与后两次之间改过提示词的粒度约束），逐条对齐发现：

| 发现                                     | A   | B   | C   | D   |
| ---------------------------------------- | --- | --- | --- | --- |
| `getConsecutiveDays` 全量拉取            | ✓   |     | ✓   | ✓   |
| `/checkin-status` 单请求多次查询         |     | ✓   | ✓   | ✓   |
| 会话列表高频全量重拉                     |     | ✓   | ✓   | ✓   |
| 流式循环 try/catch 吞掉回调异常          | ✓   | ✓   |     |     |
| `MAX_SESSION_MESSAGES` 未生效            |     | ✓   |     |     |
| `db.prepare` 未复用                      |     | ✓   |     |     |
| 前端全量历史被丢弃 + DB 重查（双重来源） |     |     | ✓   | ✓   |
| checkins 表冗余索引                      |     |     | ✓   | ✓   |
| 前端逐 token 复制整个数组                |     |     | ✓   | ✓   |

**没有任何一条出现在全部四轮里。** 出现三次的三条、两次的三条、只有一次的两条。A/B 两轮的核心是"吞异常 + SSE 背压"，C/D 两轮的核心是"双重来源 + 冗余索引 + 前端复制"——**两对之间是互补而非子集关系**。

早期从 A/B 两轮得出的"诊断稳定、粒度不稳"是**过早的结论**。加上后两轮，真实情况是：粒度确实收敛了（改后两轮对内主题 5/5 完全重合），但**发现集合本身不稳定**。

含义：**单次 `perf plan` 给出的是问题的一个部分且不可复现的子集。** 这条比粒度问题重要得多——用户跑一次拿到的只是其中一部分。

**设计。**

1. **`Step` 增加结构化位置** `locus: { file: string; range?: [number, number] }`。合并必须能机械判定"两次说的是不是同一个问题"，而当前契约里只有 `files: string[]` 加 prose 里的行号——不够。实测过 `checkin.ts` 上同时存在三条独立发现，所以"同一个文件"远不足以判定"同一个问题"。**这个字段是合并的前提，不是顺手的补充。**
2. **按位置分组**：同一 `locus.file` 且 range 相交（无 range 时要求同文件同 `kind`）的 step 归为一组；组内取一条作代表，其余变体进轨迹而不进 `plan.json`（保持 plan 干净、可人工编辑）。
3. **`Step.runCount`**：该发现在几轮里出现过。**这是实测出来的置信度，用来排序，取代模型自称的收益排序**——无证据模式下我们本来就不许它排序，`runCount` 正好补上这个位置，而它不依赖模型自觉。
4. **`--min-runs K`**：只保留出现 ≥K 轮的发现。按实测分布（3/4、2/4、1/4 各有若干），这个旋钮让用户自己决定接受多少噪声。
5. **默认 `--runs 1`**，即当前行为。**不能让默认值悄悄把成本乘三**；文档建议真实分析用 3。
6. **成本是 N 倍。** `maxTokens` 是**每轮**的上限，总成本约 N × maxTokens——文档必须写明这个乘法关系，不能只说"会慢一些"。
7. **串行执行**，进度按轮次加前缀。并发虽快，但进度会交错难读，且可能撞 API 限流。
8. **轨迹形状变化**：`.perf/trace.json` 从单个 trace 变为 `{ runs: [<每轮 trace>] }`。它是诊断产物，pre-1.0 改形状可以接受。
9. **`perf run` 的预览里显示 runCount**（如"3/3 轮均发现"），让用户在决定是否应用时看得到这条发现的证据强度。

**未决（实现前需定）：**

- **近重复怎么归并。** 同一个问题在两轮里可能被描述成位置略有差异的范围（`172-178` vs `170-180`）——按 range 相交能归并，但如果一轮说"这个函数"、另一轮说"这个函数里的某一行"，就不一定了。是否需要一个 LLM 归并步骤兜底？那会把非确定性引入合并本身。
- **代表条目的选取规则。** 暂定"rationale 最长的那条"（信息最多），但这是启发式，可能选出更啰嗦而非更准确的那条。
- **`runCount` 与 `expectedImpact` 的关系。** 有实测证据时，`runCount` 是否还该参与排序？我倾向该——它是测出来的，而 `expectedImpact` 是推断的。
...
5. **成本上限按 token 而不是按轮数。** `maxTokens`（累计输入 token）是真正起作用的上限，`maxRounds` 只是廉价兜底（防那种"每次只读几行"的模型无限循环）。

   这一条**推翻了初版设计**。初版写的是"轮数上限随有无证据而变"（有证据 10 / 无证据 8），而那个差异是我猜的——"有证据时每轮都在回答具体问题所以可以多给"与"无证据时是盲探所以该少给"两种论证都成立，我没有依据选边。所以去掉。

   **但"token 预算为主、轮数只是兜底"这个说法也不准确——实测否掉了。** pi-ai 的 `usage.input` **不含缓存命中部分**：一次 18 轮运行里每轮的 input 是 1545 / 307 / 10016 / … / 169 / 160 / 258，非单调且末轮极小，说明前缀被 provider 缓存，只有新内容算作 input。于是每轮约 4k，400k 的预算在这个 24 文件仓库上约合 100 轮——**实际生效的是轮数兜底（20）**。

   所以准确的说法是：**两个上限互补，谁先到取决于仓库规模。** 小仓库每轮新内容少 → 轮数兜底先到；大仓库每轮读进来的文件更大、未缓存输入更多 → token 预算先到。而 token 预算按"未缓存输入"计量反而是对的：缓存命中的部分便宜得多，不该按全价算进成本；副作用是不支持缓存的 provider 上会更早用尽，那是安全的方向。

   实测依据：一次 24 文件仓库的运行 9 轮累计 65k 输入 token，而它**只读了 15/24 个文件**——轮数上限决定的是"能读几个文件"（每轮约 2 个），8 轮对 24 个文件已经不够，对大仓库更不够，而那个上限还不随规模变化。
...
type Step = {
  ...
  /**
   * 结构化的位置。**`--runs N` 的合并前提**（§2.12）：判定"两轮说的是不是同一个
   * 问题"必须能机械比对，而 `files` 加 rationale 里的散文行号做不到——实测过
