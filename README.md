...
**注意：同一仓库跑两次会得到不同的发现集合，不只是措辞不同。** 模型是自主探索的，不是固定流水线。实测跑四轮，逐条对齐：三条发现在 3 轮里出现、三条在 2 轮里出现、两条只在 1 轮里出现——**没有任何一条出现在全部四轮里**。也就是说，单次运行拿到的是问题的一个子集。

想更接近全貌就用 `--runs N`（见下）。`.perf/trace.json` 记录了每轮读了什么，计划不理想时从这里看起。
...
perf plan --runs 3                             # 跑 3 轮并合并发现（⏳ 尚未实现）
```
...
> ⏳ **设计已定，尚未实现。** 下面是确定下来的行为。

单次运行只能看到问题的一部分（见上面「它怎么工作」里的实测数据）。`--runs N` 跑 N 轮、再把各轮发现合并：

- **同一处问题跨轮出现，就是置信度。** 合并后每条发现带一个 `runCount`（出现在几轮里），报告里显示为「3/3 轮均发现」。它是**测出来的**排序依据，而不是模型猜的收益排序——正好补上无实测数据时「不给排序」留下的位置。
- **默认 `--runs 1`**，即当前行为。**成本是 N 倍**（`--max-tokens` 是每轮的上限，总成本约 N × maxTokens），所以默认值不会替你把它乘三。真实分析建议 `--runs 3`。
- `--min-runs K` 只保留出现 ≥K 轮的发现。实测里三/二/一次出现的各有若干条，这个旋钮由你决定接受多少噪声。
- 各轮**串行**执行（并发虽快，但进度输出会交错，且容易撞限流）。
