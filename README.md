...
perf run --verify                              # 逐 step 验证（需先在配置里写好 verify 命令）
...
- `plan` 阶段的工具全部只读。**默认不执行你项目里的任何命令**；唯一的例外是逐 step 验证
  （`--verify`），它必须由你显式开启，且命令写在配置里。
...
**"改动能应用"与"改动是对的"是两件事。** 实测过一次：一份能干净 `git apply`、每处锚点都定位准确的补丁，仍有 2/7 个文件过不了 `tsc`、43/311 个测试失败——缺 import、引用了没定义的函数。只看"能不能应用"验不出这些。

所以建议配上验证命令，并每次带 `--verify`：

```bash
npx perf run --verify
```

行为：每个 step 写出后先跑验证命令，**通过才提交**——分支上只会累积可用的步骤。没通过就停下、报告是哪一步，失败的那个改动**留在工作区未提交**供你检查（分支本身是干净的），并给出丢弃或跳过它的做法。

命令由配置提供、由 `--verify` 武装。这是刻意的：`.perftoolrc.json` 会随仓库一起被克隆/分发，**配置里写了命令不等于你同意执行它**。`--yes` 也不会隐含 `--verify`。

代价是每个 step 跑一次验证命令，而**门槛的强度完全取决于你配什么命令**。实测过两种配置的差别：

- 只配 `npm run typecheck`：能挡住语法/类型错（挡下过一次未配对的 `}`），但**放过了**一处能通过类型检查、却让测试失败的语义退化——那是模型在计划里自己预见并接受了一个不一致（"缓存枚举结果后，探索期间新建的文件会看不到"），`tsc` 看不出这类问题。
- 配 `npm run typecheck && npm test`：慢得多（本项目上每次约 15 秒），但能挡住上面那类。

经验上：**先跑 `tsc` 那样的快命令找语法/类型问题，等你要真正合入改动时把测试也加进去**。或者干脆一次配上完整的 `npm test`，接受它慢。
...
**默认不验证改动的正确性。** 除非你配上 `verify` 并加 `--verify`，工具不会检查改完之后还能不能编译、测试还过不过。上面「逐 step 验证」一节说明了为什么这很重要。
