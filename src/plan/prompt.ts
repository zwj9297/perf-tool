...
export const buildSystemPrompt = (input: PromptInput): string => {
  ...
  sections.push(
    [
      ...
      ``,
      `**每个独立的问题一个 step。** 这些 step 将来会**逐个提交、逐个回退**——把互不相干的问题合并成一个 step，会让"只回退其中一处"变成不可能，而那正是这套流程的价值所在。实测过同一个仓库两次，同一批发现一次被塞进 2 个 step、一次拆成 5 个，导致可回退的粒度相差一倍多。`,
      `反向也成立：**同一个问题的多处改动应当放在同一个 step 里**，哪怕跨了几个文件——按文件拆开会让一个逻辑改动散在多个提交里，反而难 review。`,
      ``,
